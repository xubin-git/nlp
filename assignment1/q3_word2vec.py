...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    zz = preds.copy()
    zz[target] -= 1
    grad = np.outer(zz, vhat) #损失函数对outputVectors(即矩阵w)的偏导
    gradPred = np.dot(outputVectors.T, zz)#损失函数对vhat(即该单词的vector)的偏导
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = np.average(predicted_vectors, axis=0)
